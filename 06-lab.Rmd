...
# Finding the counts
counts <- xml2::xml_find_first(website, "//span[contains(@class, 'value')]")
...
# Making the GET request with specified query parameters
query_ids <- GET(
...
# Turning the XML content into a character string for analysis
...
# Find all the ids wrapped in <Id> tags
ids <- stringr::str_extract_all(ids, "<Id>\\d+</Id>")[[1]]

# Remove all the leading <Id> and trailing </Id> tags to isolate the numbers
...
```{r Getting-publications, eval = TRUE}
library(httr)

# Assuming `ids` is a character vector of all the ids separated by commas, e.g., "1232131,546464,13131"
# Concatenate all IDs into a single character string, separated by commas
...
publications <- GET(
  ...
  query = list(
    db = "pubmed",
    id = I(ids_string), # Use I() to prevent encoding of commas
    retmax = 300,
    rettype = "abstract"
  )
)
...
publications_content <- httr::content(publications, "text")

# Turning the output into character vector for further processing or analysis
publications_txt <- as.character(publications_content)
```
...
schools_and_deps <- str_extract_all(
  publications_txt, # Ensure this variable contains the text you're analyzing
  "\\b(School of [^\\.,;]+|Department of [^\\.,;]+)"
)
...
library(xml2)
publications_xml <- read_xml(publications_content)

# Now, use xml2 functions on the XML document
pub_char_list <- xml_children(publications_xml)
...
# Extracting abstracts, assuming they do not span multiple lines
abstracts <- str_extract(pub_char_list, "<AbstractText>(.*?)</AbstractText>")
# Cleaning all HTML tags
abstracts <- str_remove_all(abstracts, "<[^>]+>")
# Cleaning all extra whitespace and new lines
abstracts <- str_replace_all(abstracts, "\\s+", " ")
abstracts <- str_trim(abstracts) # Trims leading and trailing whitespace
num_without_abstract <- sum(is.na(abstracts))
```
```{r}
num_without_abstract
```
  - How many of these don't have an abstract? 

There are 188 publications without abstracts
...
titles <- str_trim(titles) # Trims leading and trailing whitespace
num_without_title <- sum(is.na(titles))
num_without_title
```

- How many of these don't have a title ? 

There are no articles without a title. 
...
# Use knitr::kable to create a basic table
library(knitr)
kable_output <- kable(database, "html")  # Specifying "html" for compatibility with kableExtra

# To make the output more manageable, especially if it's large, use kableExtra::scroll_box
library(kableExtra)
kable_output %>% 
